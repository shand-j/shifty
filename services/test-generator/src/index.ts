import {
  DatabaseManager,
  TestGenerationRequestsRepository,
} from "@shifty/database";
import {
...
class TestGeneratorService {
  ...
  private async registerRoutes() {
    ...
    fastify.post("/api/v1/tests/generate", async (request, reply) => {
      try {
        ...
        this.processTestGeneration(requestId, body).catch((error) => {
          ...
          this.updateGenerationStatus(requestId, "failed", undefined, {
            error: error.message,
          });
        });
        ...
        reply.send({
          ...
          status: "pending",
          message:
            "Test generation started. Use /status endpoint to check progress.",
        });
      } catch (error) {
    ...
    fastify.get(
      "/api/v1/tests/generate/:requestId/status",
      async (request, reply) => {
        try {
          // Try JWT auth first, fall back to header
          let tenantId = extractTenantFromAuth(request);
          ...
          const generationRequest = await this.getGenerationRequest(
            requestId,
            tenantId
          );
    ...
    fastify.post("/api/v1/tests/:requestId/improve", async (request, reply) => {
      try {
        ...
        const originalTest = await this.getGenerationRequest(
          requestId,
          tenantId
        );
  ...
  private async createGenerationRequest(
    ...
    try {
      ...
      console.error(
        `Failed to persist generation request ${id} for tenant ${tenantId}:`,
        error
      );
  ...
  private async updateGenerationStatus(
    ...
    try {
      ...
      if (!tenantId) {
        console.warn(
          `No tenant ID found for request ${id}, status update skipped`
        );
        return;
      }
  ...
  private async getGenerationRequest(
    ...
    try {
      ...
      console.error(
        `Failed to get generation request ${id} for tenant ${tenantId}:`,
        error
      );
  ...
  private async getGenerationHistory(
    ...
    try {
      const dbUrl = this.getTenantDbUrl(tenantId);
      const records = await this.testGenRepo.getHistory(
        tenantId,
        dbUrl,
        limit,
        offset
      ...
      console.error(
        `Failed to get generation history for tenant ${tenantId}:`,
        error
      );
      return [];
    }
