...
export class DataIngestionService {
  ...
  constructor() {
    this.cicdGovernorUrl =
      process.env.CICD_GOVERNOR_URL || "http://localhost:3007";
    this.roiServiceUrl = process.env.ROI_SERVICE_URL || "http://localhost:3008";
    this.manualSessionHubUrl =
      process.env.MANUAL_SESSION_HUB_URL || "http://localhost:3009";
  }
  ...
  async ingestTenantData(tenantId: string): Promise<KnowledgeDocument[]> {
    ...
    console.log(
      `Ingested ${documents.length} documents for tenant ${tenantId}`
    );
    return documents;
  }
   ...
   */
  private async ingestSessionData(
    tenantId: string
    ...
    try {
      ...
      return sessions.map((session: any) => ({
        ...
        lastUpdated: new Date(
          session.endTime || session.startTime || Date.now()
        ),
      }));
    } catch (error) {
   ...
   */
  private async ingestTelemetryData(
    tenantId: string
  ): Promise<KnowledgeDocument[]> {
    try {
      const response = await axios.get(
