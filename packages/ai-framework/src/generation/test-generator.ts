...
export class AITestGenerator {
  ...
  async generate(options: TestGenerationOptions): Promise<GeneratedTest> {
  ...
  private buildPrompt(options: TestGenerationOptions, pageAnalysis: any): string {
...
Include only the test code, no explanations.`;
  ...
  private async callAI(prompt: string, provider: string, model?: string): Promise<string> {
  ...
  private parseAIResponse(response: string, options: TestGenerationOptions): GeneratedTest {
  ...
  private extractSelectors(code: string): Array<{ selector: string; confidence: number; alternatives: string[] }> {
    const selectors: Array<{ selector: string; confidence: number; alternatives: string[] }> = [];
  ...
  private calculateSelectorConfidence(selector: string): number {
  ...
  private calculateConfidence(selectors: Array<{ confidence: number }>): number {
    if (selectors.length === 0) return 0.3;
  ...
  private getFallbackTest(): string {
