...
class TestGenerator {
    ...
    async improveTest(originalCode, feedback) {
        ...
        if (feedback.toLowerCase().includes('slow')) {
            improvedCode = improvedCode.replace(/page\.waitForTimeout\(\d+\)/g, 'page.waitForLoadState("networkidle")');
        }
    ...
    async enhanceWithAI(baseCode, request) {
        try {
