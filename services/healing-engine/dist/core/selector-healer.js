...
class SelectorHealer {
    ...
    async healSelector(page, brokenSelector, expectedElementType, preferredStrategy) {
    ...
    async tryStrategy(page, brokenSelector, strategy, expectedElementType) {
        switch (strategy) {
    ...
    async tryDataTestIdRecovery(page, brokenSelector) {
        try {
    ...
    async tryCssHierarchyAnalysis(page, brokenSelector) {
        try {
            const alternatives = [];
            const pageUrl = page.url();
    ...
    async tryAiAnalysis(page, brokenSelector, expectedElementType) {
        try {
        ...
        try {
            return await page.evaluate(() => {
                const elements = Array.from(document.querySelectorAll('*'))
                    .filter((el) => el.children.length === 0) // Leaf elements
                    .slice(0, 10)
                    .map((el) => ({
