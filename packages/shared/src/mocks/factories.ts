...
export function generateMockUsers(count: number = 200): MockUser[] {
  ...
  const personas: MockUser["persona"][] = [
    "developer",
    "qa",
    "product",
    "manager",
...
export function generateMockProjects(
  ...
  const frameworks: MockProject["framework"][] = [
    "playwright",
    "cypress",
    "selenium",
    "jest",
  ...
  for (let i = 0; i < count; i++) {
    ...
    const passingTests = faker.number.int({
      min: Math.floor(testCount * 0.5),
      max: testCount,
    });
    const failingTests = faker.number.int({
      min: 0,
      max: testCount - passingTests,
    });
...
export function generateMockTests(
  count: number = 5000,
  projects: MockProject[],
  users: MockUser[]
): MockTest[] {
  const tests: MockTest[] = [];
  const statuses: MockTest["status"][] = [
    "passing",
    "failing",
    "flaky",
    "skipped",
  ];
  const priorities: MockTest["priority"][] = [
    "critical",
    "high",
    "medium",
    "low",
...
export function generateMockHealingSuggestions(
  ...
  const statuses: MockHealingSuggestion["status"][] = [
    "pending",
    "approved",
    "rejected",
    "applied",
  ...
  for (let i = 0; i < count; i++) {
    ...
    suggestions.push({
      ...
      selectorType: faker.helpers.arrayElement([
        "css",
        "xpath",
        "testid",
        "text",
...
export function generateMockPipelines(
  ...
  const providers: MockPipeline["provider"][] = [
    "github",
    "gitlab",
    "jenkins",
    "circleci",
  ...
  const statuses: MockPipeline["status"][] = [
    "success",
    "failure",
    "running",
    "pending",
  ...
  for (let i = 0; i < count; i++) {
    ...
    const testsPassed = faker.number.int({
      min: Math.floor(testsRun * 0.7),
      max: testsRun,
    });
...
export function generateMockKnowledgeEntries(
  count: number = 1000,
  users: MockUser[]
): MockKnowledgeEntry[] {
...
export function generateMockArcadeMissions(
  count: number = 50
): MockArcadeMission[] {
  const missions: MockArcadeMission[] = [];
  const difficulties: MockArcadeMission["difficulty"][] = [
    "beginner",
    "intermediate",
    "advanced",
    "expert",
...
export function generateMockDORAMetrics(days: number = 90): MockDORAMetric[] {
  ...
  for (let i = 0; i < days; i++) {
    ...
    metrics.push({
      ...
      changeFailureRate: faker.number.float({
        min: 0,
        max: 0.15,
        fractionDigits: 3,
      }),
