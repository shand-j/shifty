...
exports.MigrationRunner = exports.DatabaseManager = void 0;
...
class DatabaseManager {
    ...
    async getTenantPool(tenantId, databaseUrl) {
    ...
    async queryTenant(tenantId, databaseUrl, text, params) {
    ...
    async tenantTransaction(tenantId, databaseUrl, callback) {
        const pool = await this.getTenantPool(tenantId, databaseUrl);
...
class MigrationRunner {
    ...
    `);
        ...
        for (const migration of migrations) {
            const result = await this.dbManager.query('SELECT id FROM migrations WHERE name = $1', [migration.name]);
            if (result.rows.length === 0) {
                console.log(`📝 Running migration: ${migration.name}`);
                await this.dbManager.query(migration.sql);
                await this.dbManager.query('INSERT INTO migrations (name) VALUES ($1)', [migration.name]);
                console.log(`✅ Migration completed: ${migration.name}`);
            }
        }
        console.log('✅ Platform migrations completed');
    }
    async runTenantMigrations(tenantId, databaseUrl) {
    ...
    `);
        ...
        for (const migration of migrations) {
            const result = await this.dbManager.queryTenant(tenantId, databaseUrl, 'SELECT id FROM migrations WHERE name = $1', [migration.name]);
            if (result.rows.length === 0) {
                console.log(`📝 Running tenant migration: ${migration.name}`);
                await this.dbManager.queryTenant(tenantId, databaseUrl, migration.sql);
                await this.dbManager.queryTenant(tenantId, databaseUrl, 'INSERT INTO migrations (name) VALUES ($1)', [migration.name]);
