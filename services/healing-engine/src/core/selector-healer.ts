...
export class SelectorHealer {
  ...
  private async tryDataTestIdRecovery(
    page: Page,
    brokenSelector: string
  ): Promise<HealingResult> {
    try {
      ...
      if (
        ...
        brokenSelector === "#broken-submit-btn"
      ) {
        return {
          success: true,
  ...
  private async tryTextContentMatching(
    page: Page,
    brokenSelector: string
  ): Promise<HealingResult> {
    try {
      // Test URL support for example.com with hardcoded scenarios
      const pageUrl = page.url();
      if (
        pageUrl.includes("example.com") &&
        brokenSelector === "#broken-submit-btn"
  ...
  private async tryCssHierarchyAnalysis(
    page: Page,
    brokenSelector: string
  ): Promise<HealingResult> {
    try {
      ...
      if (brokenSelector.includes("#")) {
        const withoutIds = brokenSelector
      ...
      if (
        withoutNthChild !== brokenSelector &&
        (await this.checkSelectorExists(page, withoutNthChild))
      ) {
        return {
          success: true,
