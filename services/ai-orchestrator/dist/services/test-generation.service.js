...
class TestGenerationService {
    constructor(ollamaService, dbManager) {
        this.ollama = ollamaService;
        this.dbManager = dbManager;
        this.defaultModel = process.env.AI_MODEL || 'llama3.1';
    }
    async generateTest(tenantId, request) {
    ...
    async improveTest(tenantId, existingTest, feedback) {
...
`;
        try {
            ...
            const confidence = this.calculateConfidence(response, { testType: 'e2e' });
            await this.logTestGeneration(tenantId, {
                description: 'Test improvement',
                testType: 'e2e'
            }, improvedCode, confidence);
            return {
                code: improvedCode,
...
No explanations, just the complete, runnable test code.`;
    ...
    getTestTypeGuidelines(testType) {
        switch (testType) {
            case 'e2e':
...
- High code coverage`;
    ...
    extractTestCode(response) {
        ...
        const codeStart = lines.findIndex(line => line.includes('import') ||
            line.includes('test(') ||
    ...
    extractSuggestions(response) {
        ...
        if (suggestions.length === 0) {
            const sentences = response.split(/[.!?]/)
                .map(s => s.trim())
                .filter(s => s.length > 20 && (s.toLowerCase().includes('consider') ||
    ...
    calculateConfidence(response, request) {
        ...
        if (response.response.includes('import') && response.response.includes('test('))
            confidence += 0.2;
        if (response.response.includes('await') && response.response.includes('page.'))
            confidence += 0.1;
        if (response.response.includes('expect('))
            confidence += 0.1;
        if (response.response.includes('data-testid') || response.response.includes('[data-test'))
            confidence += 0.1;
        ...
        if (request.testType === 'smoke')
            confidence += 0.1;
        if (request.testType === 'e2e' && request.pageUrl)
            confidence += 0.05;
        return Math.min(confidence, 0.95); // Cap at 95%
    }
    estimateTestDuration(testType, testCode) {
        ...
        let duration = baseDurations[testType] || 30;
    ...
    async logTestGeneration(tenantId, request, generatedCode, confidence) {
        try {
