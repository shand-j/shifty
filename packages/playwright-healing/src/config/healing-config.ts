 ...
 */
export function loadHealingConfig(overrides: Partial<HealingConfig> = {}): HealingConfig {
  // Start with defaults
...
function mergeConfigs(
  ...
  return {
    ...
    ollama: {
      ...
      model: override.ollama?.model ?? base.ollama?.model ?? 'llama3.1:8b',
      timeout: override.ollama?.timeout ?? base.ollama?.timeout ?? 30000,
    },
    retry: {
      onTimeout: override.retry?.onTimeout ?? base.retry?.onTimeout ?? true,
      onFlakiness: override.retry?.onFlakiness ?? base.retry?.onFlakiness ?? true,
      maxRetries: override.retry?.maxRetries ?? base.retry?.maxRetries ?? 2,
      initialBackoff: override.retry?.initialBackoff ?? base.retry?.initialBackoff ?? 1000,
    },
    telemetry: {
      enabled: override.telemetry?.enabled ?? base.telemetry?.enabled ?? true,
      logLevel: override.telemetry?.logLevel ?? base.telemetry?.logLevel ?? 'info',
    },
  };
}
 ...
 */
export function validateConfig(config: HealingConfig): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  if (config.maxAttempts !== undefined && (config.maxAttempts < 1 || config.maxAttempts > 10)) {
  ...
  if (config.retry?.initialBackoff !== undefined && config.retry.initialBackoff < 100) {
    errors.push('retry.initialBackoff must be at least 100ms');
  }

  // Validate log level
  const validLogLevels = ['debug', 'info', 'warn', 'error'];
  if (config.telemetry?.logLevel && !validLogLevels.includes(config.telemetry.logLevel)) {
