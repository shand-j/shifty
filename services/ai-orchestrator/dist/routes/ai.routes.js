...
const TestGenerationSchema = zod_1.z.object({
    description: zod_1.z.string().min(1),
    testType: zod_1.z.enum(['e2e', 'integration', 'unit', 'smoke']),
    framework: zod_1.z.enum(['playwright', 'cypress', 'webdriver']).optional(),
    pageUrl: zod_1.z.string().url().optional(),
    userStory: zod_1.z.string().optional(),
    acceptanceCriteria: zod_1.z.array(zod_1.z.string()).optional()
});
const TestImprovementSchema = zod_1.z.object({
    existingTest: zod_1.z.string().min(1),
    feedback: zod_1.z.string().min(1)
});
const SelectorHealingSchema = zod_1.z.object({
    brokenSelector: zod_1.z.string().min(1),
...
const BatchHealingSchema = zod_1.z.object({
    requests: zod_1.z.array(SelectorHealingSchema).min(1).max(10)
});
async function aiRoutes(fastify, options) {
    ...
    fastify.post('/api/v1/ai/improve-test', async (request, reply) => {
        try {
            ...
            const result = await testGenerationService.improveTest(tenantId, body.existingTest, body.feedback);
            reply.send({
                success: true,
                data: result,
                timestamp: new Date().toISOString()
